let translate path =
  ...
  Ortac_default.Translate.signature ~context sigs

let mutability =
  let open Ortac_default.Translated in
...
let get_mutability = function
  | Ortac_default.Translated.Type t -> t.mutable_
  | _ -> assert false

let is_pure = function
  | Ortac_default.Translated.Value v -> v.pure
  | _ -> assert false

let type_name = function
  | Ortac_default.Translated.Type t -> t.name
  | _ -> assert false

let val_name = function
  | Ortac_default.Translated.Value v -> v.name
  | _ -> assert false

let mutability_to_string = function
  | Ortac_default.Translated.Mutable -> "mutable"
  | Ortac_default.Translated.Unknown -> "unknown"
  | Ortac_default.Translated.Dependant _ -> "dependant"
  | Ortac_default.Translated.Immutable -> "immutable"

let is_val (t : Ortac_default.Translated.structure_item) =
  match t with Ortac_default.Translated.Value _ -> true | _ -> false

let test_mutability path mut flag () =
  let translations = translate path in
  Ortac_default.Translated.iter_translation
    ~f:(fun t ->
      if !flag then (
...
let type_unknown () =
  ...
  test_mutability "./translation/unknown.mli" Ortac_default.Translated.Unknown flag

let type_mutable () =
  let flag = ref false in
  test_mutability "./translation/mutable.mli" Ortac_default.Translated.Mutable flag

let type_immutable () =
  let flag = ref false in
  test_mutability "./translation/immutable.mli" Ortac_default.Translated.Immutable
    flag

let type_dependant () =
  let flag = ref false in
  test_mutability "./translation/dependant.mli"
    (Ortac_default.Translated.Dependant (fun _ -> Ortac_default.Translated.Unknown))
    flag
