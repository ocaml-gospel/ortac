    ...
    | expr ->
        error
          ( Impossible_init_state_generation
              (Not_a_function_call
                 (Fmt.str "%a@." Ppxlib.Pprintast.expression expr)),
            Ppxlib.Location.none )
  in
  let fct_str =
    match fct with
    | Ppxlib.Lident s | Ppxlib.Ldot (_, s) -> s
    | Ppxlib.Lapply (_, _) -> assert false
  in
  let is_init_declaration = function
    | { sig_desc = Sig_val (v, _); _ }
      when Fmt.str "%a" Ident.pp v.vd_name = fct_str ->
        Some v
    | _ -> None
  in
  let* value =
    ...
    |> of_option ~default:(No_init_function fct_str, Ppxlib.Location.none)
  in
  let* spec =
    of_option
      ~default:
      ...
      value.vd_spec
  in
  let arguments = List.combine spec.sp_args args in
