...
type config_under_construction = {
  ...
  gen_cmd' : Ppxlib.structure_item option;
  gen_cmd_seq' : Ppxlib.structure_item option;
  gen_cmd_dom0' : Ppxlib.structure_item option;
  gen_cmd_dom1' : Ppxlib.structure_item option;
...
let mk_config context module_prefix submodule domain count cfg_uc =
  ...
  and gen_cmd = cfg_uc.gen_cmd'
  and gen_cmd_seq = cfg_uc.gen_cmd_seq'
  and gen_cmd_dom0 = cfg_uc.gen_cmd_dom0'
  and gen_cmd_dom1 = cfg_uc.gen_cmd_dom1'
...
let value_bindings cfg_uc =
  ...
  let aux cfg_uc vb =
    ...
    | Ppat_var s when String.equal "gen_cmd" s.txt ->
        let gen_cmd' =
          Option.some @@ Ortac_core.Builder.pstr_value Nonrecursive [ vb ]
        in
        ok { cfg_uc with gen_cmd' }
    | Ppat_var s when String.equal "gen_cmd" s.txt ->
        let gen_cmd_seq' =
          Option.some @@ Ortac_core.Builder.pstr_value Nonrecursive [ vb ]
        in
        ok { cfg_uc with gen_cmd_seq' }
    | Ppat_var s when String.equal "gen_cmd_dom0" s.txt ->
        let gen_cmd_dom0' =
          Option.some @@ Ortac_core.Builder.pstr_value Nonrecursive [ vb ]
        in
        ok { cfg_uc with gen_cmd_dom0' }
    | Ppat_var s when String.equal "arb_cmd_dom1" s.txt ->
        let gen_cmd_dom1' =
          Option.some @@ Ortac_core.Builder.pstr_value Nonrecursive [ vb ]
        in
        ok { cfg_uc with gen_cmd_dom1' }
    | _ -> ok cfg_uc
  in
