...
module S =
  struct
    let set = declare_abstract_type ~var:"set" ()
    let t = declare_abstract_type ~var:"t" ()
  end
let () =
  (((((((let spec = M.int ^!> S.set in
      ...
      (let spec = M.int ^!> S.t in
       declare "create_int is Ok" spec R.create_int C.create_int));
     (let spec = M.int ^!> S.t in
      declare "bad_create_int is Ok" spec R.bad_create_int C.bad_create_int));
    (let spec = S.t ^!> M.int in
     declare "increment_int is Ok" spec R.increment_int C.increment_int));
   (let spec = S.t ^!> M.int in
    declare "bad_increment_int is Ok" spec R.bad_increment_int
      C.bad_increment_int));
  (let spec = S.t ^!> M.int in
   declare "bad2_increment_int is Ok" spec R.bad2_increment_int
     C.bad2_increment_int)
