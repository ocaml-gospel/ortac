...
let epred e = eapply (evar "Runtime.Z.pred") [ e ]
let esucc e = eapply (evar "Runtime.Z.succ") [ e ]

let econst = function
  | Pconst_integer (c, o) ->
      Pconst_integer (c, o) |> pexp_constant |> fun e ->
      eapply (evar "Runtime.Z.of_int") [ e ]
...
let elocation loc =
  [%expr
    {
      Runtime.start = [%e eposition loc.loc_start];
      Runtime.stop = [%e eposition loc.loc_end];
    }]
